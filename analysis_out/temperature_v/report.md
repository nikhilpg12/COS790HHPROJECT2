...
_Read-only analysis. Generated by `python -m llm_gp_hh.analysis`. The experiment execution code paths are not imported; only the pure constructor / parser / tree / reference-table modules are._

**5 run directories** — 4 complete, 0 mid-run, 1 empty.

## Validity gates (per run)

A failed gate means the corresponding number is **suppressed** below, not shown with a caveat.

| run | feasible | SCV vs pub | timing | provenance | replicates |
| --- | --- | --- | --- | --- | --- |
...
**Cross-run:** 880 candidates over 4 runs → 647 unique raw trees, 640 unique canonical trees.

Most repeated tree: `(* (+ a b) (- c d))` — 11 occurrences.
...
| car-f-92 | 1.000 | 8bfda5177179 | 1 | 0 |  |  |  | n=1 — not ranked |

_No cell has ≥2 complete runs. Nothing can be ranked — every temperature currently has a single seed (finding 7)._

## Run issues

- **results/temperature/T_1.0/car-f-92-20260831T180221Z-seed1001** (empty): no output files (empty or not-yet-started run directory)
